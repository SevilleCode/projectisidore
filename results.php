...
if (isset($_POST['search'])) {
  $colname_searches = $_POST['search'];
}
$query_searches = sprintf("SELECT * FROM sources WHERE Description LIKE %s", GetSQLValueString("%" . $colname_searches . "%", "text"));
$searches = $myData->query($query_searches) or die($myData->error);
$row_searches = $searches->fetch_assoc();
$totalRows_searches = $searches->num_rows;
?>
<?php include('header.php') ?>
    <div class="jumbotron jumbotron-fluid text-center">
       ...
       <hr class="my-4">
		<?php do { ?>
		<p class="text-left"><?php echo $row_searches['Source']; ?><br>
		  <a href="<?php echo $row_searches['URL']; ?>" target="_blank"><?php echo $row_searches['URL']; ?></a><br>
		  <?php echo $row_searches['Description']; ?></p>
		<?php } while ($row_searches = $searches->fetch_assoc()); ?>
        <?php if ($totalRows_searches == 0) { // Show if recordset empty ?>
  <p>Sorry, we couldn't find anything matching those search terms. Please try again!</p>
